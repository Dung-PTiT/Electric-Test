            ...
            if (mailContent != "") {
                $.ajax({
                    ...
                    success: function () {
                        ...
                        swal("Xong", "Gửi mail thành công", "success");
                    }, error: function () {
                        swal("Cảnh báo", "Không thành công", "warning");
                    }
                });
            }
        }
        ...
        submitHandler: function () {
            ...
            if (customerID == "") {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        swal("Xong", "Tìm thành công", "success");
                        if (data[0][10] == 'active') {
                            var contentString = "";
                            for (var i = 0; i < data.length; i++) {
                                contentString = contentString
                                    ...
                                    + '<td>' + data[i][4] + '</td>'
                                    + '<td style="padding: 0;">' + data[i][5] + '</td>'
                                    + '<td>' + data[i][6] + '</td>'
                                    + '<td>' + data[i][7] + '</td>'
                                    + '<td>' + data[i][8] + '</td>'
                                    + '<td>' + data[i][9] + '</td>'
                                    + '<td><a href="#" data-toggle="tooltip" title="Send Email" onclick="getIDKH()"><button data-toggle="modal" data-target="#SendMailModal" class="btn btn-primary center-block mb-1" style="padding: 1px 1px 1px 1px;"><i class="icon-mail-read"></i></button></a>' +
                        ...
                        if (data[0][10] == 'disable') {
                            var contentString = "";
                            for (var i = 0; i < data.length; i++) {
                                contentString = contentString
                                    + '<tr role="row" class="odd" style="color: #c7c7c7">'
                                    + '<td id="id_KH_td">' + data[i][0] + '</td>'
                                    + '<td>' + data[i][1] + '</td>'
                                    + '<td>' + data[i][2] + '</td>'
                                    + '<td>' + data[i][3] + '</td>'
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data[0][1] != tenKH_update || data[0][2] != dob_update || data[0][3] != soCmnd_update || data[0][4] != diaChi_update ||
                        ...
                        $('.nav__edit-customer').hide();
                        $('.overlay_bang_khach_hang').hide();
                        swal("Cảnh báo", "Không có thông tin nào được cập nhật", "warning");
                    }
                }, error: function (data) {
                    swal("Cảnh báo", data.responseText, "warning");
                }
