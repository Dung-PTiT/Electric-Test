...
function showTable(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (status == 0) {
            ...
            var billStatus = '<td style="color: red; text-align: center">' + status + '</td>';
        } else {
            status = "Đã lập hóa đơn";
            var billStatus = '<td style="color: green; text-align: center">' + status + '</td>';
        }
        contentString = contentString
            ...
            + '<td>' + data[i][1] + '</td>'
            + '<td>' + data[i][2] + '</td>'
            + '<td style="text-align: end">' + data[i][3] + '</td>'
            + '<td style="text-align: end">' + data[i][4] + '</td>'
            + billStatus
...
function taoHoaDon() {
    ...
    console.log(maDK + " " + maHD + " " + maKH + " " + maThang + " " + luongDienTT + " " + loaiDienSD);
    $.ajax({
        url: "/lap-hoa-don",
        type: "POST",
        data: {
